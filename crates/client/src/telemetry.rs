...
use std::fs::File;
use std::{env, path::PathBuf, sync::{Arc, LazyLock}, time::{Duration, Instant}};
...
struct TelemetryState {
    ...
    _release_channel: Option<ReleaseChannel>,
    _architecture: &'static str,
    events_queue: Vec<EventWrapper>,
    _flush_events_task: Option<Task<()>>,

    log_file: Option<File>,
    is_staff: Option<bool>,
    _first_event_date_time: Option<Instant>,
...
#[cfg(debug_assertions)]
const _FLUSH_INTERVAL: Duration = Duration::from_secs(1);

#[cfg(not(debug_assertions))]
const _FLUSH_INTERVAL: Duration = Duration::from_secs(60 * 5);
...
impl Telemetry {
    pub fn new(
        ...
        let state = Arc::new(Mutex::new(TelemetryState {
            ...
            _architecture: env::consts::ARCH,
            _release_channel: ReleaseChannel::try_global(cx),
            system_id: None,
            installation_id: None,
            session_id: None,
            metrics_id: None,
            events_queue: Vec::new(),
            _flush_events_task: None,
            log_file: None,
            is_staff: None,
            _first_event_date_time: None,
            event_coalescer: EventCoalescer::new(clock.clone()),
            _max_queue_size: MAX_QUEUE_LEN,
